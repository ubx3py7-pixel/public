...
async def flush(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if not is_owner(user_id):
        ...
        return
    global users_tasks, persistent_tasks, gc_tasks_global
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if chat_type == Chat.PRIVATE:
        if is_authorized_user(user_id):
            welcome_text += "✅ You are authorized! Type /help to see available commands"
        else:
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    help_text += " /login 📱 - Login to Instagram account\n"
    help_text += " /plogin 🔐 - Playwright human-like login\n"
    help_text += " /slogin 🔑 - Login with session ID\n"
    help_text += " /viewmyac 👀 - View your saved accounts\n"
    help_text += " /setig 🔄 <number> - Set default account\n"
    help_text += " /pair 📦 ig1-ig2 - Create account pair for rotation\n"
    help_text += " /unpair ✨ - to unpair paired accounts (unpair all or unpair <account>)\n"
    help_text += " /switch ⏱️ <min> - Set switch interval (5+ min)\n"
    
    if chat_type == Chat.PRIVATE:
        help_text += " /threads 🔢 <1-5> - Set number of threads\n"
    else:
        help_text += " /threads 🔢 <1> - Set number of threads (1 thread only in groups)\n"
    
    help_text += " /viewpref ⚙️ - View preferences\n"
    help_text += " /attack 💥 - Start sending messages\n"
    help_text += " /stop 🛑 <pid/all> - Stop tasks\n"
    help_text += " /task 📋 - View ongoing tasks\n"
    help_text += " /logout 🚪 <username> - Logout and remove account\n"
    ...
    if is_owner(user_id):
        ...
        help_text += " /remove ➖ <tg_id> - Remove authorized user or group\n"
...
def load_persistent_tasks():
    global persistent_tasks, gc_tasks_global
...
async def list_users(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if authorized_groups:
        for i, group_id in enumerate(authorized_groups, 1):
            msg += f"{i}. Group ID: {group_id}\n"
    else:
        msg += "No authorized groups.\n"
...
def main_bot():
    ...
    print(f"👑 Owner ID: {OWNER_TG_ID}")
    print(f"📢 Required channel: {REQUIRED_CHANNEL}")
    print(f"👥 Max GC tasks: {MAX_GC_TASKS}")
